...
with market_base as (
    ...
    where try_cast(event_time as timestamp) >= (select max(try_cast(event_time as timestamp)) - interval '3 days' from "crypto"."main"."slv_decision_features")
    ...
    from "crypto"."main"."br_stock_prices"
...
sentiment_base as (
    ...
    from "crypto"."main"."br_news_sentiment"
    group by 1, 2
),
x_sentiment_base as (
    select
    ...
    from "crypto"."main"."br_x_sentiment"
    group by 1
),
joined as (
    select
