...
with market as (
    ...
    from {{ source('raw', 'market_snapshot_raw') }}
    {% if is_incremental() %}
    where try_cast(event_time as timestamp) > (select max(try_cast(event_time as timestamp)) from {{ this }})
    {% endif %}
),
derivatives as (
    select
    ...
    from {{ source('raw', 'derivatives_snapshot_raw') }}
    {% if is_incremental() %}
    where try_cast(event_time as timestamp) > (select max(try_cast(event_time as timestamp)) from {{ this }})
    {% endif %}
),
...
cross_ex as (
    ...
    from (
        select event_time, exchange, symbol, cast(last_price as double) as raw_last, cast(bid_price as double) as raw_bid, cast(ask_price as double) as raw_ask, cast(quote_volume_24h as double) as raw_quote from {{ source('raw', 'cross_exchange_snapshot_raw') }}
        {% if is_incremental() %}
        where try_cast(event_time as timestamp) > (select max(try_cast(event_time as timestamp)) from {{ this }})
        {% endif %}
        union all
